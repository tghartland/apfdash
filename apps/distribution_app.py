...
def generate_binned_plot_1s():
    ...
    empty4_hist = go.Bar(x=dataframe["remotewallclocktime"], y=dataframe["empty4_jobs"], name="Empty (completed)", marker={"color":"#EF751A"}, width=1)
    empty3_hist = go.Bar(x=dataframe["remotewallclocktime"], y=dataframe["empty3_jobs"], name="Empty (removed)", marker={"color":"#C21E29"}, width=1)
    ...
    data = [empty3_hist, empty4_hist, payload_hist]
...
def generate_binned_plot_10m():
    ...
    empty4_hist = go.Bar(x=dataframe["minutes"], y=dataframe["empty4_jobs"], name="Empty (completed)", marker={"color":"#EF751A"}, width=1)
    empty3_hist = go.Bar(x=dataframe["minutes"], y=dataframe["empty3_jobs"], name="Empty (removed)", marker={"color":"#C21E29"}, width=1)
